...
import { compile, Options } from './main';
...
async function processDir(
  ...
  results.forEach(([file, result, outputPath]) =>
    outputResult(
      result,
      outputPath ? `${outputPath}/${basename(file, '.json')}.d.ts` : undefined,
    ),
  );
}
...
function getPaths(path: string, paths: string[] = []) {
  if (existsSync(path) && lstatSync(path).isDirectory()) {
    readdirSync(resolve(path)).forEach((item) =>
      getPaths(join(path, item), paths),
    );
  } else {
    paths.push(path);
  }
...
export function readInput(argIn?: string) {
  if (!argIn) {
